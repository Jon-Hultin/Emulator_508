...
import com.jagex.js5.util.crypto.RSACipher;
...
/**
 * A {@link JS5ChecksumTable} stores checksums and versions of
 * {@link JS5ReferenceTable}s. When encoded in a {@link JS5Container} and prepended
 * with the file type and id it is more commonly known as the client's
 * "update keys".
 * @author Graham
 * @author `Discardedx2
 */
public final class JS5ChecksumTable {

	/**
	 * Decodes the {@link JS5ChecksumTable} in the specified
	 * {@link ByteBuffer}. Whirlpool digests are not read.
	 * @param buffer The {@link ByteBuffer} containing the table.
	 * @return The decoded {@link JS5ChecksumTable}.
	 * @throws IOException if an I/O error occurs.
	 */
	public static JS5ChecksumTable decode(ByteBuffer buffer) throws IOException {
	 ...
	 * @throws IOException if an I/O error occurs.
	 */
	public static JS5ChecksumTable decode(ByteBuffer buffer, boolean whirlpool) throws IOException {
	 ...
	 * @throws IOException if an I/O error occurs.
	 */
	public static JS5ChecksumTable decode(ByteBuffer buffer, boolean whirlpool, BigInteger modulus, BigInteger publicKey) throws IOException {
		...
		if (whirlpool) {
			...
			if (modulus != null && publicKey != null) {
				temp = RSACipher.crypt(buffer, modulus, publicKey);
			}
	...
	/**
	 * Represents a single entry in a {@link JS5ChecksumTable}. Each entry
	...
	/**
	 * Creates a new {@link JS5ChecksumTable} with the specified size.
	 * @param size The number of entries in this table.
	 */
	public JS5ChecksumTable(int size) {
		entries = new Entry[size];
	}

	/**
	 * Encodes this {@link JS5ChecksumTable}. Whirlpool digests are not encoded.
	...
	/**
	 * Encodes this {@link JS5ChecksumTable}.
	...
	/**
	 * Encodes this {@link JS5ChecksumTable} and encrypts the final whirlpool hash.
	 * @param whirlpool If whirlpool digests should be encoded.
	 * @param modulus The modulus.
		...
		try (DataOutputStream os = new DataOutputStream(bout)) {
			...
			if (whirlpool) {
				...
				if (modulus != null && privateKey != null) {
					temp = RSACipher.crypt(temp, modulus, privateKey);
				}
