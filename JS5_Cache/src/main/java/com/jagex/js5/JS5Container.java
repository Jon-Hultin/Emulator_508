...
import com.jagex.js5.util.CompressionUtils;
import com.jagex.js5.util.crypto.XteaBlockCipher;

import java.io.IOException;
import java.nio.ByteBuffer;

/**
 * A {@link JS5Container} holds an optionally compressed file. This class can be
 ...
 * @author `Discardedx2
 */
public final class JS5Container {
	 ...
	 */
	public static JS5Container decode(ByteBuffer buffer) throws IOException {
		return JS5Container.decode(buffer, NULL_KEY);
	}

	public static JS5Container decode(ByteBuffer buffer, int[] key) throws IOException {
		...
		if (key[0] != 0 || key[1] != 0 || key[2] != 0 || key[3] != 0) {
			XteaBlockCipher.decipher(buffer, 5, length + (type == COMPRESSION_NONE ? 5 : 9), key);
		}
		...
		if (type == COMPRESSION_NONE) {
			...
			return new JS5Container(type, data, version);
		} else {
			...
			/* and return the decoded container */
			return new JS5Container(type, ByteBuffer.wrap(uncompressed), version);
		}
	}
	 ...
	 */
	public JS5Container(int type, ByteBuffer data) {
		this(type, data, -1);
	}
	 ...
	 */
	public JS5Container(int type, ByteBuffer data, int version) {
		this.type = type;
		this.data = data;
